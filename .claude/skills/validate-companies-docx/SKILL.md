...
description: "ALWAYS use this skill (instead of validate-companies) whenever a .docx file is involved. Use when the user wants to validate Israeli company references inside a Word (.docx) file and annotate the document with correct registration details from the Israeli Companies Registrar (רשם החברות הישראלי). Triggers on: ANY request that references a .docx file path AND involves company validation, annotation, or completion — such as 'check companies in this contract', 'annotate the Word file with company numbers/name/registered address', 'fill in company details in the document', 'validate companies in the agreement', 'add company registration numbers to the Word file', 'complete the company details in the docx', 'fix the company info in the Word file'. Also triggers when the user provides a .docx file path and asks to complete, verify, annotate, or fix company information — even if they do not explicitly mention tracked changes. This skill MUST be used instead of validate-companies whenever a .docx file is the target; validate-companies is only for plain text or non-docx content. For Hebrew documents, assume the company is an Israeli company unless mentioned otherwise. For other languages use only if instructed or if context suggests the company is incorporated or registered in Israel."
---
